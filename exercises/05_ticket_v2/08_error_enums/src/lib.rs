...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TitleError("Title cannot be empty".into()));
        }
        if title.len() > 50 {
            return Err(TitleError("Title cannot be longer than 50 bytes".into()));
        }
        if description.is_empty() {
            return Err(DescriptionError("Description cannot be empty".into()));
        }
        if description.len() > 500 {
            return Err(DescriptionError("Description cannot be longer than 500 bytes".into()));
        }
